...
describe("init", () => {
  ...
  it("사과와 뱀은 항상 패널 범위 안에 배치된다 (randInt 범위 검증)", () => {
    for (let i = 0; i < 200; i++) {
      const state = game.init(5, 5);
      const appleCount = state.applePanel
        .flat()
        .filter((c) => c.color === "red").length;
      const snakeCount = state.snakePanel
        .flat()
        .filter((c) => c.color === "orange").length;
      assert.strictEqual(
        appleCount,
        1,
        `${i}번째 init에서 사과가 범위 밖에 배치됨`,
      );
      assert.strictEqual(
        snakeCount,
        1,
        `${i}번째 init에서 뱀이 범위 밖에 배치됨`,
      );
    }
  });
});
...
describe("tick", () => {
  ...
  it("경계 밖으로 나가면 뱀 head가 더 이상 이동하지 않는다", () => {
    // 5×5 보드에서 up 방향으로 벽에 충분히 붙인 뒤
    // 추가 tick에도 위치가 고정되는지 확인한다.
    const state = game.init(5, 5);
    let s = game.key("up", state);
    for (let i = 0; i < 10; i++) s = game.tick(s);

    const posAfter10 = getSnakeItems(s)
      .map((c) => `${c.row},${c.column}`)
      .sort();

    for (let i = 0; i < 3; i++) s = game.tick(s);

    const posAfter13 = getSnakeItems(s)
      .map((c) => `${c.row},${c.column}`)
      .sort();
    ...
    assert.deepStrictEqual(
      posAfter13,
      posAfter10,
      "경계 도달 후 추가 tick에서도 뱀 위치가 변하지 않아야 한다",
    );
  });
