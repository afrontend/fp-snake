...
const isOverlapItem = (apple, snake) => isNotBlank(apple) && isNotBlank(snake);
const isOverlap = (applePanel, snakePanel) => {
  return _.some(
    _.zipWith(to1DimAry(applePanel), to1DimAry(snakePanel), isOverlapItem),
    fp.isEqual(true)
  );
};
...
const justPaintSnake = posAry =>
  paint(createPanel(), posAry, GLOBAL.snakeColor);
...
const getSnake = _.flow([to1DimAry, fp.filter(isNotBlank)]);
...
const assignPanel = ({ applePanel, snakePanel }) => {
  ...
  return _.chunk(
    _.zipWith(to1DimAry(applePanel), to1DimAry(snakePanel), zipPanelItem),
    columns
  );
};
