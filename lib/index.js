...
const makeRemembered = (fn) => {
  let savedRows = 0;
  let savedColumns = 0;
  return (rows, columns) => {
    savedRows = rows ?? savedRows;
    savedColumns = columns ?? savedColumns;
    return fn(savedRows, savedColumns);
  };
};

const createPanel = makeRemembered((r, c) => p.createPanel(r, c));
const createApplePanel = makeRemembered((r, c) =>
  paintApple(createPanel(r, c)),
);
const createSnakePanel = makeRemembered((r, c) =>
  paintSnake(createPanel(r, c)),
);
...
const justPaintSnake = (posAry) =>
  p.paint(createPanel(), posAry, config.snakeColor);

const moveSnake = _.flow([
  _.flattenDepth,
  fp.filter(p.isFilled),
  fp.sortBy("index"),
  addHeadItem,
  reindex,
  removeTailItem,
...
const getSnake = _.flow([_.flattenDepth, fp.filter(p.isFilled)]);
